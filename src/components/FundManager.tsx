...
export function FundManager() {
  const [accountId, setAccountId] = useState<InvestAccountId>('personal-isa');
  const [modelId, setModelId] = useState('growth-aggressive');
  const [pot, setPot] = useState(50_000);
  const [showConstraints, setShowConstraints] = useState(true);

  const account = INVEST_ACCOUNTS.find((a) => a.id === accountId)!;

  const onAccountChange = (id: InvestAccountId) => {
    const acc = INVEST_ACCOUNTS.find((a) => a.id === id)!;
    setAccountId(id);
    setModelId(acc.suggestedModelId);
    setPot(acc.defaultPot);
  };

  const rows = useMemo(
    () => allocate(modelId, pot, account.channel),
    [modelId, pot, account.channel]
  );
  ...
  return (
    <section className="fund-manager">
      <div className="section-heading">
        <h2>Fund manager</h2>
        <p>
          Mirror best-in-class passive portfolios. UK accounts use UCITS ETFs; your US
      ...
      <div className="fm-toolbar">
        <label>
          ...
          <select
            value={accountId}
            onChange={(e) => onAccountChange(e.target.value as InvestAccountId)}
          >
        ...
        <label>
          Pot to deploy ({account.currency})
          <input
            type="number"
            value={pot}
      ...
      <div className="fm-channel-banner">
        {account.channel === 'uk' ? (
          <>
            <strong>UK implementation</strong> — UCITS ETFs on LSE (ISA / SIPP / GIA eligible).
            Use accumulating share classes where available.
          </>
        ) : (
          <>
            <strong>US implementation</strong> — individual common stocks only. Basket
            approximates index weights; extend with more names over time.
          </>
        )}
      </div>
      ...
      <div className="fm-table-wrap">
        <table className="fm-table">
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={`${row.ticker}-${row.name}`} className={row.ticker === '—' ? 'row-note' : ''}>
                <td className="col-ticker">{row.ticker}</td>
                <td>{row.name}</td>
                <td className="col-num">{row.weight.toFixed(1)}%</td>
                <td className="col-num">{formatMoney(row.amount, account.currency)}</td>
      ...
      <div className="fm-checklist">
        ...
        <ol>
          ...
          <li>Log trades for US/UK tax reporting (IRS + HMRC if applicable).</li>
        </ol>
      </div>
